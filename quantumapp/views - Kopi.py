...
def deploy_contract(request):
    if request.method == 'POST':
        try:
            ...
            abi = json.loads(contract_data['abi']) if isinstance(contract_data['abi'], str) else contract_data['abi']
            bytecode = contract_data['bin']
            w3 = Web3(Web3.HTTPProvider('https://161.35.219.10:32788'))
            w3.middleware_onion.inject(geth_poa_middleware, layer=0)

            # Check if connected
            if not w3.is_connected():
                logger.error('Could not connect to Polygon CDK node.')
                return JsonResponse({'error': 'Could not connect to Polygon CDK node.'}, status=500)

            # Use pre-funded account for deployment
            pre_funded_account = '0xE34aaF64b29273B7D567FCFc40544c014EEe9970'
            pre_funded_private_key = '0x12d7de8621a77640c9241b2595ba78ce443d05e94090365ab3bb5e19df82c625'
            account = w3.eth.account.from_key(pre_funded_private_key)
            ...
            try:
                ...
                nonce = w3.eth.get_transaction_count(initial_owner)

                # Estimate gas
                estimated_gas = Web3Contract.constructor(token_name, token_symbol, initial_supply).estimate_gas({'from': initial_owner})
                ...
                transaction = {
                    ...
                    'gasPrice': gas_price,
                    'nonce': nonce,
                    'data': Web3Contract.constructor(token_name, token_symbol, initial_supply).data_in_transaction,
                    'chainId': chain_id
                }

                signed_txn = w3.eth.account.sign_transaction(transaction, private_key=pre_funded_private_key)
...
def home(request):
    ...
    wallets = Wallet.objects.exclude(user__username='system')  # Exclude the system wallet
    context = {
        'form': form,
        'wallets': wallets,
    }
